...
class ItemItemCF(PersonCorrelation):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.usr_unwatched_movies = self.all_data[self.all_data[Config.main_user_id].isnull()].index.values
    ...
    def calculate_rating_prediction(self, main_id):
        ...
        """
        neighbors = self.get_item_item_neighbors(main_id)
        score = 0
        similarity_sum = 0
    ...
    def get_best_movies(self):
        ...
        for i in range(1, Config.num_of_movies+1):
            ...
            print(best_movie)
            movie, year = ItemItemCF._get_movie_and_year(imdb, best_movie)
